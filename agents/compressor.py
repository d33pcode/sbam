...
def compress(path):
	...
	if (os.path.exists(archive_path)):
		replace = questions.queryYesNo("WARNING: you already have today's backup for this folder. Do you want to replace it?", default="no")
		if not replace:
	...
	with tarfile.open(archive_path, mode='w:bz2') as archive:
		...
		for subdir, dirs, files in os.walk(path):
			for f in files:
				logging.debug('Adding ' + os.path.join(subdir, f) + ' to the archive.')
